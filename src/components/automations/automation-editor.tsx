...
import { ArrowLeft, Folder, FolderOpen, Globe, Wand2 } from "lucide-react"
...
export function AutomationEditor({
  ...
  // Desktop: native directory picker. Web/server mode has no filesystem access
  // to the server, so `openFileDialog` falls back to a typed-path prompt there.
  const browseLocalFolder = async () => {
    try {
      const picked = await openFileDialog({
        directory: true,
        title: t("localFolderBrowse"),
      })
      if (typeof picked === "string" && picked) setLocalFolderPath(picked)
    } catch {
      // Dialog unavailable or dismissed — leave the typed path as-is.
    }
  }
          ...
          <div className="flex flex-col gap-2">
            <div className="flex flex-wrap items-center gap-2">
              <Input
                ...
                placeholder={t("localFolderPathPlaceholder")}
                aria-label={t("targetLocalFolder")}
