...
> **Commit context:** working tree at `3745933d`; plugin layer committed in `aec0b9ff`
> **Date:** 2026-08-04
> **Method:** Qartez graph + codebase-memory (cbm) semantic search + targeted reads of every plugin file and every engine extension surface. All citations are `file:line` from the committed tree.

---

## Objective

Determine, from evidence, whether a large personal AI-assistant platform can be built **primarily through Codeg's plugin/extension layer** while staying compatible with future upstream Codeg releases — or whether the architecture forces a long-term fork.

Primary question, answered in §Anatomy–Verdict: **hybrid — buildable via plugins, with a small (3-seam) engine touch surface; fork only under specific conditions.**

---

## Verified Facts

### A. The plugin layer (source-out-of-tree, compile-time coupled)

1. **Backend plugin files are git-tracked, out-of-tree Rust modules.** `git ls-files plugins/` → `backend/{mod.rs, custom_auto_approve.rs, custom_cron.rs, web_workflows.rs}`, `frontend/{custom-page.tsx, automation-editor.tsx}`, `vendor.patch`. They are **committed** (part of `aec0b9ff`), not gitignored.
2. **Mount mechanism** — `src-tauri/src/lib.rs:15`: `#[path = "../../plugins/backend/mod.rs"] pub mod custom_plugins;`. The plugin compiles *into* the engine crate and addresses engine internals via `crate::` paths: `crate::db::AppDatabase`, `crate::acp::manager::ConnectionManager`, `crate::acp::types::PromptInputBlock`, `crate::db::entities::conversation`, `crate::app_state::AppState` (all confirmed in the four backend files).
3. **Intent, documented in-code** — `plugins/backend/mod.rs:1-7`: "kept outside the upstream crate tree so they survive upstream merges"; Tauri shims "feature-gated: `codeg-server` builds without `tauri-runtime`".
4. **`plugins/backend/mod.rs` (107 lines)** — 6 `#[cfg(feature = "tauri-runtime")] #[tauri::command]` shims: `toggle_auto_approve_global` :19-37, `get_auto_approve_global` :40-53, `save_custom_workflow` :66-71, `delete_custom_workflow` :74-78, `list_custom_workflows` :80-85, `set_custom_workflow_enabled` :89-93, `run_custom_workflow_now` :97-105; serde response shapes `AutoApproveToggleResult`/`AutoApproveGetResult` :56-64; `pub use custom_cron::CustomWorkflow;` :107.
5. **Auto-approve plugin** — `plugins/backend/custom_auto_approve.rs`: `AUTO_APPROVE_KEY = "auto_approve_global"` persisted in the engine's `app_metadata` KV store :15; `OnceLock<Mutex<Option<bool>>>` cache, **fail-closed** (:21-25: `None` ⇒ treated as disabled); `AutoApproveError` (NotFound/PermissionDenied/Internal) :28-36; `map_db_error` :38-40; accessors `is_auto_approved_sync` :44-49, `is_auto_approved` :54-56.
6. **Workflow scheduler plugin** — `plugins/backend/custom_cron.rs` (425 lines): JSON store `custom_workflows.json` in the data dir beside SQLite :37; `CustomCronEngine` struct :232-238, `new` :241-247, `run` :250-257, `tick` :259-293 (30s interval), `fire` :299-302 → fires via `ConnectionManager::send_prompt_linked_with_message_id`; **owns no approval state**; FILE_LOCK serializes read-modify-write.
7. **Server-mode shims** — `plugins/backend/web_workflows.rs` (94 lines): Axum handlers (`SaveWorkflowRequest`, `WorkflowIdRequest`, `SetWorkflowEnabledRequest`); `storage_error` → `AppErrorCode::IoError` :40-46; `custom_workflow_list` :49-51; delegates to `custom_cron`.
8. **Frontend plugin files** — `plugins/frontend/custom-page.tsx` (731 lines): `CustomWorkflowsPage` master–detail UI; doc comment :75-86 states "Custom Workflows (custom plugin). Backed by the `custom_cron` scheduler"; consumes engine UI kit (`@/components/ui/*`, `@/lib/api`, `@/lib/utils`). `plugins/frontend/automation-editor.tsx` (488 lines): **verbatim copy** of the native Automations editor with the folder target swapped for a target-conversation picker (explicit in comments :38-43, :720-722); emits `WorkflowDraft = {name, conversation_id, cron, prompt}` :62-67.
9. **Frontend mount points (engine files already carrying the plugin route):**
   - `src/components/workbench/workbench-content.tsx:13` — `import { CustomWorkflowsPage } from "../../../plugins/frontend/custom-page"`; `:25` — `"custom-workflows": CustomWorkflowsPage` in `WORKBENCH_ROUTES`.
...
4. **KV store** — `app_metadata` service: the persistence primitive the auto-approve plugin leverages (`custom_auto_approve.rs:15`); generic, already engine-native.
5. **Emit path** — `src-tauri/src/web/event_bridge.rs` `EventEmitter` enum is the **sole** event-emission channel; grep for `custom`/`CustomWorkflow` → **0 hits**. The plugin layer currently emits no events (command/handler-driven only). This is a genuine extension ceiling (see Gaps).
...
- **`plugins/backend/*` has 0 import edges in the qartez import graph** — the `#[path]` cross-tree mount is invisible to file-import indexing. Only frontend edge resolves: `workbench-content.tsx → custom-page.tsx → automation-editor.tsx`. Engineering implication: the plugin layer is a blind spot for any import-graph tooling.
...
**Data flow (desktop):** UI (`custom-page.tsx`) → `lib/api.ts` RPC (`getTransport().call`) → `invoke()` → Tauri command (shim in `plugins/backend/mod.rs`) → business logic in `plugins/backend/custom_cron.rs` / `custom_auto_approve.rs` → engine `AppDatabase`/`ConnectionManager`/`app_metadata`. **Server:** same UI → `fetch()` → Axum handler in `web_workflows.rs` → same plugin logic.
...
2. **No plugin event emission** — grep of `event_bridge.rs` returns zero custom-channel hits. Push-style plugin features (progress, notifications, permission-request UI) have no native lane; they'd need a new seam.
...
4. **Graph blindness** — `plugins/backend/*` invisible to import-graph tooling (qartez 0 edges). Any future code-health tooling must special-case the `#[path]` mount.
...
1. (Recommended) Keep plugin architecture; add a **seam re-application checklist** to `agent.md` for post-upstream-pull workflow.
...
- **Ownership:** engine = `src-tauri/src/**`, `src/**`, `src/i18n/**` (never edit); custom = `plugins/backend/**`, `plugins/frontend/**` (edit freely); `plugins/patches/**` = read-only archive.
...
- **Registration mechanics:** commands must be added in `lib.rs` (engine) but can live in `plugins/backend/mod.rs` as `#[tauri::command]` re-exports; routes can be added in `plugins/backend/web_workflows.rs` but must be registered in `router.rs` (engine).
- **Lifecycle:** engine tokio runtime owns all plugin async; plugin state via `OnceLock`/`Mutex` + engine `AppState`.
- **Data flows:** single transport abstraction (`getTransport().call`) covers desktop `invoke` + server `fetch`; plugin RPCs must be mirrored in `lib/api.ts`.
- **Merge strategy:** upstream pulls land on engine files; the 5 seams are the only manual re-application points; `plugins/backend` + `plugins/frontend` merge cleanly by virtue of being plugin-dev-only paths.
